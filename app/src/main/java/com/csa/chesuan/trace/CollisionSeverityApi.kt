...
object CollisionSeverityApi {
    ...
    private fun localSeverityPrediction(
        ...
        if (bundle.event.type != AccidentType.COLLISION) {
            return predictNonCollisionOperationalRisk(
                event = bundle.event,
                metrics = metrics,
...
private const val NON_COLLISION_SERIOUS_THRESHOLD = 58

private fun predictNonCollisionOperationalRisk(
    event: AccidentEvent,
    metrics: ResponsibilityAnalyzer.DetailedMetrics,
    derivedFeatures: CollisionSeverityDerivedFeatures,
): CollisionSeverityPredictionResult {
    var score = when (event.type) {
        AccidentType.TTC_LOW_RISK -> 55
        AccidentType.AEB_DELAY_OR_MISS -> 52
        AccidentType.DRIVER_SLOW_REACTION -> 48
        AccidentType.MULTI_FACTOR -> 50
        AccidentType.DRIVER_TAKEOVER_FAIL -> 42
        AccidentType.AUTOPILOT_FAULT -> 38
        AccidentType.ENVIRONMENT_DISTURB -> 35
        else -> 40
    }
    ...
    if (metrics.reactionTimeMs > 1200) score += 12
    else if (metrics.reactionTimeMs > 800) score += 6
    if (metrics.ttcAtBrakeStart in 0f..1.5f) score += 10
    else if (metrics.ttcAtBrakeStart in 0f..2.5f) score += 5
    if (metrics.maxDecelerationMS2 <= -8f) score += 8
    if (metrics.avgSpeedLast3sKph >= 60f) score += 6
    if (metrics.brakeEffective) score -= 4
    score = score.coerceIn(25, 72)

    val probabilities = nonCollisionProbabilities(score)
    val predicted = probabilities.maxByOrNull { it.value }?.key ?: "Slight"
    val fatalPct = ((probabilities["Fatal"] ?: 0.0) * 100).roundToInt()
    val seriousPct = ((probabilities["Serious"] ?: 0.0) * 100).roundToInt()
    val slightPct = ((probabilities["Slight"] ?: 0.0) * 100).roundToInt()

    val narrative = buildString {
        append("当前事件类型并非碰撞（COLLISION），伤亡分类模型不参与主判；")
        append("以下为基于事件子类、触发等级与遥测信号（反应时间、TTC、减速度等）")
        append("计算的运营风险严重度，用于近失、AEB、跟车等场景的风险分层展示。")
        append(" 运营风险指数 ").append(score).append("/100，映射为")
        append(predicted.toDisplaySeverityLabel())
        append("（Fatal ").append(fatalPct).append("% / Serious ")
            .append(seriousPct).append("% / Slight ").append(slightPct).append("%）。")
    }

    val factors = buildList {
        add("推断模式：operational_risk（非碰撞），事件子类=${event.type.name}")
        add("触发等级=${event.severity.name}")
        if (metrics.reactionTimeMs > 0) add("反应时间 ${metrics.reactionTimeMs}ms")
        if (metrics.ttcAtBrakeStart >= 0f) {
            add("TTC 约 ${format1(metrics.ttcAtBrakeStart)}s")
        }
        if (metrics.maxDecelerationMS2 <= -8f) {
            add("峰值减速度 ${format2(metrics.maxDecelerationMS2)}m/s²")
        }
        add("概率分布：Fatal ${fatalPct}% / Serious ${seriousPct}% / Slight ${slightPct}%")
    }.take(6)

    return CollisionSeverityPredictionResult(
        predictedSeverity = predicted,
        probabilities = probabilities,
        modelHint = "collision_severity · operational_risk-v1 · 非碰撞运营风险分级 · 端侧离线",
        derivedFeatures = derivedFeatures,
        severityScore = score,
        narrative = narrative,
        keyFactors = factors,
...
private fun nonCollisionProbabilities(score: Int): LinkedHashMap<String, Double> {
    val serious: Double
    val slight: Double
    val fatal: Double
    if (score >= NON_COLLISION_SERIOUS_THRESHOLD) {
        serious = minOf(0.72, 0.48 + (score - NON_COLLISION_SERIOUS_THRESHOLD) * 0.025)
        fatal = 0.05
        slight = maxOf(0.15, 1.0 - serious - fatal)
    } else {
        slight = minOf(0.88, 0.68 + (NON_COLLISION_SERIOUS_THRESHOLD - score) * 0.008)
        serious = maxOf(0.08, 0.28 - (NON_COLLISION_SERIOUS_THRESHOLD - score) * 0.006)
        fatal = maxOf(0.04, 1.0 - slight - serious)
    }
    val total = (slight + serious + fatal).coerceAtLeast(0.0001)
    return linkedMapOf(
        "Fatal" to fatal / total,
        "Serious" to serious / total,
        "Slight" to slight / total,
    )
}
