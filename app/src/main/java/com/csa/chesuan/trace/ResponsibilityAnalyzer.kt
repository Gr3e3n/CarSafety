...
object ResponsibilityAnalyzer {
    ...
    fun inferResponsibility(
        ...
        if (event.type == AccidentType.AUTOPILOT_FAULT || event.type == AccidentType.DRIVER_TAKEOVER_FAIL) driverScore -= 15

        // 系统分
        var systemScore = 15
        if (event.type == AccidentType.AUTOPILOT_FAULT || event.type == AccidentType.DRIVER_TAKEOVER_FAIL) systemScore += 30
        if (metrics.aebDelayMs in -5000..-500) systemScore += 15
        val collisionLike = event.type == AccidentType.COLLISION ||
            event.type == AccidentType.DRIVER_SLOW_REACTION ||
            event.type == AccidentType.AEB_DELAY_OR_MISS ||
            event.type == AccidentType.TTC_LOW_RISK ||
            event.type == AccidentType.ENVIRONMENT_DISTURB ||
            event.type == AccidentType.MULTI_FACTOR
        if (metrics.aebDelayMs == -1 && collisionLike) systemScore += 10
        ...
        val conclusion = when (event.type) {
            ...
            AccidentType.AUTOPILOT_FAULT, AccidentType.DRIVER_TAKEOVER_FAIL -> when {
