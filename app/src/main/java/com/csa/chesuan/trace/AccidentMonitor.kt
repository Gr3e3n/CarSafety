...
import com.csa.chesuan.db.AccidentDao
import com.csa.chesuan.db.AccidentDatabase
import com.csa.chesuan.db.AccidentEventEntity
import com.csa.chesuan.db.ResponsibilityEntity
import com.csa.chesuan.db.TelemetryEntity
...
object AccidentMonitor {
    ...
    /** 环形缓冲阶段无具体事件，用占位事件驱动与 CarExt 语义对齐的扩展遥测合成。 */
    private val monitorSyntheticEvent = AccidentEvent(
        id = "LIVE-SYNTH",
    ...
    private var lastSteerDeg: Float = 0f
    ...
    private suspend fun persistToRoom(detail: AccidentDetailBundle) {
        ...
        d.insertTelemetry(
            (detail.telemetry10sBefore + detail.telemetry10sAfter).map { p ->
                TelemetryEntity(
                    eventId = e.id,
                    tMs = p.tMs,
                    speedKph = p.speedKph,
                    ...
                    steerDeg = p.steerDeg,
                    ayMS2 = p.ayMS2,
                    yawRateDegS = p.yawRateDegS,
                    throttlePct = p.throttlePct,
                    aebActive = if (p.aebActive) 1 else 0,
                    blinkerCode = p.blinkerCode,
                    fcwActiveLevel = p.fcwActiveLevel,
                )
            }
        )
        val resp = detail.responsibility
        val metrics = ResponsibilityAnalyzer.analyze(
            e,
            detail.telemetry10sBefore,
            aebTriggerTMs = detail.telemetry10sBefore.firstOrNull { it.aebActive }?.tMs,
        )
    ...
    private fun buildDetail(
        ...
        val triggers = when (type) {
            ...
            -> listOf("碰撞加速度超阈值", "紧急制动+急减速")
            AccidentType.AUTOPILOT_FAULT, AccidentType.DRIVER_TAKEOVER_FAIL -> listOf("自动驾驶异常退出", "控制模块响应异常")
        }
        val event = AccidentEvent(
            ...
            autoDrivingState = when (type) {
                ...
                AccidentType.AUTOPILOT_FAULT, AccidentType.DRIVER_TAKEOVER_FAIL -> AutoDrivingState.AUTONOMOUS
            },
            summary = when (type) {
                AccidentType.COLLISION,
                AccidentType.DRIVER_SLOW_REACTION,
                ...
                AccidentType.MULTI_FACTOR,
                -> "检测到急减速与强制动，触发自动取证"
                AccidentType.AUTOPILOT_FAULT, AccidentType.DRIVER_TAKEOVER_FAIL -> "检测到自动驾驶异常与控制延迟，触发故障溯源"
            },
        )
        val telemetry10sBefore = normalizePreWindow(pre)
        val telemetry10sAfter = normalizePostWindow(post)
        val env = AccidentContextGenerator.environmentFor(event)
        val trace = if (type == AccidentType.AUTOPILOT_FAULT || type == AccidentType.DRIVER_TAKEOVER_FAIL) {
        ...
        val metrics = ResponsibilityAnalyzer.analyze(
            event,
            telemetry10sBefore,
            aebTriggerTMs = telemetry10sBefore.firstOrNull { it.aebActive }?.tMs,
        )
        ...
        return AccidentDetailBundle(
            ...
            telemetry10sAfter = telemetry10sAfter,
    ...
    private fun normalizePostWindow(post: List<TelemetryPoint>): List<TelemetryPoint> {
        if (post.isEmpty()) return post
        val firstT = post.first().tMs
        return post.map { it.copy(tMs = it.tMs - firstT) }
    }
    ...
    private fun sampleTelemetry(tMs: Int): TelemetryPoint {
        val isFirstInWindow = (tMs == -PRE_WINDOW_MS)
        if (isFirstInWindow) lastSteerDeg = 0f
        ...
        val point = TelemetrySignalSynthesizer.enrich(
            event = monitorSyntheticEvent,
            tMs = tMs,
            dtMs = SAMPLE_INTERVAL_MS.toInt(),
            speedKph = speed,
            axMS2 = ax,
            brake = brake,
            steerDeg = steer,
            prevSteerDeg = lastSteerDeg,
            isFirstSample = isFirstInWindow,
        )
        lastSteerDeg = steer
        return point
    }
