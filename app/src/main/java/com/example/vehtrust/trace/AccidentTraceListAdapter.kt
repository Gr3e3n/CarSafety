...
import com.example.vehtrust.databinding.LayoutTraceListHeaderBinding
    ...
    inner class HeaderViewHolder(private val binding: LayoutTraceListHeaderBinding) :
        ...
        fun bind(list: List<AccidentEvent>) {
            ...
            binding.tvTraceEventCountBadge.text = binding.root.context.getString(
                com.example.vehtrust.R.string.trace_event_count_badge,
                count,
            )

            binding.tvTraceMonitorState.text = if (last == null) {
                binding.root.context.getString(com.example.vehtrust.R.string.trace_monitor_on)
            } else {
                val t = shortFormatter.format(Date(last.timeMillis))
                binding.root.context.getString(com.example.vehtrust.R.string.trace_monitor_last, t)
            }
            ...
            if (last != null) {
                ...
                binding.tvTraceLastEventTitle.text =
                    binding.root.context.getString(com.example.vehtrust.R.string.trace_last_event_none)
                binding.tvTraceLastEventMeta.text =
                    binding.root.context.getString(com.example.vehtrust.R.string.trace_last_event_none_hint)
            }
