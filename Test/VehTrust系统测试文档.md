...
| 2 | 模块卡片点击 | 点击"驾驶辅助"等安全模块卡片，返回后点击"事故溯源" | 安全模块进入详情页；"事故溯源"进入列表页 |
| 3 | 事故列表展示 | 从主页进入事故溯源列表 | 列表正常渲染，展示事件 ID、时间、位置、原因，至少 1 条记录 |
| 4 | 详情页加载 | 点击列表中任一事故条目 | 详情页完整渲染所有功能模块，无白屏或数据缺失 |
| 5 | 遥测回放操作 | 拖动进度条，点击播放、暂停 | 3D 场景与数据实时同步更新，自动播放时进度条匀速推进 |
| 6 | 碰撞严重度预测 | 断开网络后点击生成预测 | 3 秒内返回分类、置信度、严重度指数、概率分布和关键因素 |
| 7 | AI 在线分析 | 启动后端服务，确保网络连通，点击 AI 分析 | 返回包含摘要、责任、分析、证据、建议、模型的完整结构化结果 |
| 8 | AI 离线降级 | 关闭后端服务，再次点击 AI 分析 | 显示离线降级提示，返回本地模板化结果，不崩溃不白屏 |
| 9 | 地图加载 | 进入详情页观察地图区域 | 交互式地图正常加载，事故位置标记点可见 |
| 10 | 区块链存证成功 | 启动链服务，点击上链按钮 | 显示交易哈希和时间戳，按钮变为"已存证" |
| 11 | 区块链存证失败 | 关闭链服务，点击上链按钮 | 10 秒内显示超时错误提示，重试按钮可用 |
...
| 序号 | 测试场景 | 测试过程描述 | 测试结果 |
|------|---------|-------------|---------|
| 1 | 屏蔽用户误操作 | 2 秒内连续点击"AI 分析"按钮 10 次 | 仅发起一次请求，后续点击被拦截，UI 保持响应 |
| 2 | 错误正确提示 | 后端返回 HTTP 500 错误 | 显示"服务异常"提示并自动降级，不弹窗报错 |
| 3 | 异常输入不崩溃 | 传入不存在的 eventId | 显示"事件未找到"提示或正常退出 |
| 4 | 空数据处理 | 遥测数据为空或部分字段缺失时进入详情 | 对应区域显示"暂无数据"，不出现空指针异常 |
| 5 | 后台恢复 | 详情页按 Home 键，5 分钟后切回 | 前台服务通知仍在，页面数据完整，无 ANR |
...
| 序号 | 测试场景 | 测试过程描述 | 测试结果 |
|------|---------|-------------|---------|
| 1 | 操作指导信息 | 检查各页面是否具有必要的引导信息 | 各页面标题清晰，模块名称准确，导航路径明确 |
| 2 | 界面风格一致性 | 在各页面之间切换，观察视觉风格 | 色彩体系统一，卡片圆角和阴影风格一致，字体层级分明 |
| 3 | 屏幕方向适配 | 在模拟器中旋转屏幕方向 | 旋转后布局自适应，关键信息仍可见可操作 |
| 4 | 触控尺寸合理性 | 使用模拟器鼠标点击各按钮和可交互元素 | 所有可点击元素可准确触发，无误触 |
| 5 | 错误提示可理解性 | 触发各类异常场景，阅读错误提示文案 | 文案通俗易懂，包含故障原因和操作建议 |
...
| 序号 | 页面 | 目标值 | 实测值 |
|------|------|--------|--------|
| 1 | 主页（MainActivity） | < 2s | |
| 2 | 事故列表（AccidentTraceActivity） | < 1s | |
| 3 | 事故详情（AccidentTraceDetailActivity） | < 2s | |
...
|------|------|------|---------|---------|--------|
| 1 | 碰撞严重度预测 | 本地 ML | — | < 3s | |
| 2 | AI 事故分析（在线） | 远程 API | 180s | 5~60s | |
| 3 | AI 降级分析（离线） | 本地规则 | — | < 2s | |
| 4 | 区块链上链 | 远程 API | 10s | < 10s | |
...
（1）**功能完整性**：基本功能测试 11 项，通过 \_\_\_ 项，通过率 \_\_\_%。

（2）**系统稳定性**：可靠性测试 5 项，通过 \_\_\_ 项。无 Crash 和 ANR，异常处理全部正确响应。

（3）**用户体验**：可用性测试 5 项，通过 \_\_\_ 项。界面风格一致，操作引导清晰，错误提示可理解。

（4）**性能表现**：页面加载、API 响应、内存占用、帧率均在目标指标范围内。
